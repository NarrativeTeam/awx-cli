...
class JobTemplateEditCommand(JobTemplateBaseCommand.JobTemplateBaseCommand):

    """ edit job template """

    def __init__(self, toplevel):
        super(JobTemplateEditCommand, self).__init__(toplevel)
        self.name = "jobtemplateedit"

    def get_parser(self):
        parser = super(JobTemplateEditCommand, self).get_parser()
        parser.add_option('--id', type="int")
        return parser

    def get_missing_option_errors(self, options):
        # Only an id required when editing.
        errors = []
    ...
    def make_requests(self, handle, options):
        data = self.get_request_data_no_defaults(options)
